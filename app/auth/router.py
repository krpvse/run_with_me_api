...
from app.auth.services import UsersService, RunnersService
...
async def register(user_data: SUserReg):
    ...
    user_id = await UsersService.add(email=user_data.email, hashed_password=hashed_password)
    await RunnersService.add(user_id=user_id, name=user_data.name)
    return {'user_id': user_id, 'email': user_data.email, 'name': user_data.name}


@router.post('/login')
async def login(response: Response, user_data: SUserAuth):
    user = await UsersService.find_one_or_none(email=user_data.email)
